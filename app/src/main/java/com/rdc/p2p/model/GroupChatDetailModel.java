 ...
 *    2020.6
 */
public class GroupChatDetailModel implements GroupChatDetailContract.Model {

    private static final String TAG = "ChatDetailModel";
    private GroupChatDetailContract.Presenter mPresenter;
    ...
    public GroupChatDetailModel(GroupChatDetailContract.Presenter presenter, GroupBean groupBean) {
        ...
        //TODO 需要获取所有成员的SocketThread，逐一发送
        threadList = new ArrayList<>();
        for (PeerBean peerBean:mGroupBean.getPeerBeanList()) {
            if(peerBean.getUserIp().equals(App.getMyIP())){
                continue;
            }
            SocketThread socketThread = SocketManager.getInstance().getSocketThreadByIp(peerBean.getUserIp());
            Log.d(TAG, "当前全部SocketThread成员：" + peerBean.getNickName());
            if(socketThread != null) {
                Log.d(TAG, "当前保持的SocketThread：" + peerBean.getNickName());
    ...
    public void exit() {
        for (PeerBean peerBean:mGroupBean.getPeerBeanList()) {
            SocketManager.getInstance().removeSocketThreadByIp(peerBean.getUserIp());
            SocketManager.getInstance().removeSocketByIp(peerBean.getUserIp());
        }
    }
